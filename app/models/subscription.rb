...
class Subscription < ApplicationRecord
  ...
  has_one :subscription_execution, -> { order(created_at: :desc) }, dependent: :destroy, inverse_of: :subscription
  ...
  def self.search_fields = { status: { node: -> { arel_table[:status] }, type: :string }, **base_search_fields, **User.associated_search_fields }
  ...
  def create_execution!
    ...
    service.steps.order(:position).each do |step|
  ...
  def enqueue_step_executions!(execution, scheduled_at:)
    ...
    execution
      ...
      .each do |step_execution|
        ...
        break if step_execution.status == "in_progress" || step_execution.scheduled_job?
        break if scheduled_at + step_execution.step.offset_seconds.seconds > Time.zone.now
  ...
  def enqueue_step_execution!(execution, step_execution)
    ...
    perform_later(
      StepEvaluateJob,
      arguments: { step_execution: step_execution },
