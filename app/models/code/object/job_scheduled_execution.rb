...
class Code
  class Object
    class JobScheduledExecution < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::JobScheduledExecution).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::JobScheduledExecution).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::JobScheduledExecution.column_names).empty?
