...
class Code
  class Object
    class Country < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::Country).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::Country).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::Country.column_names).empty?
          raise ::Code::Error, "invalid_record_attributes"
        end

        Pundit
          .policy_scope!(::Current.user, ::Country)
          .where(attributes)
          .to_code
      end
      ...
      def self.code_find(value)
        ...
        Pundit.policy_scope!(::Current.user, ::Country).find_by(id: id).to_code
      end

      def self.code_find!(value)
        id = value.to_code.to_s
        Pundit.policy_scope!(::Current.user, ::Country).find(id).to_code
      end
      ...
      def country! = policy_scope(::Country).find(id)
      def code_user = policy_scope(::User).find(country!.user_id).to_code
