...
class Code
  class Object
    class JobRecurringExecution < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::JobRecurringExecution).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::JobRecurringExecution).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::JobRecurringExecution.column_names).empty?
