...
class Code
  class Object
    class CountryCodeIpAddress < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::CountryCodeIpAddress).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::CountryCodeIpAddress).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::CountryCodeIpAddress.column_names).empty?
          raise ::Code::Error, "invalid_record_attributes"
        end

        Pundit
          .policy_scope!(::Current.user, ::CountryCodeIpAddress)
          .where(attributes)
          .to_code
      end
