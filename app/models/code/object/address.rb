...
class Code
  class Object
    class Address < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::Address).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::Address).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::Address.column_names).empty?
          raise ::Code::Error, "invalid_record_attributes"
        end

        Pundit
          .policy_scope!(::Current.user, ::Address)
          .where(attributes)
          .to_code
      end
      ...
      def self.code_find(value)
        ...
        Pundit.policy_scope!(::Current.user, ::Address).find_by(id: id).to_code
      end

      def self.code_find!(value)
        id = value.to_code.to_s
        Pundit.policy_scope!(::Current.user, ::Address).find(id).to_code
      end
      ...
      def address! = policy_scope(::Address).find(id)
      def code_user = policy_scope(::User).find(address!.user_id).to_code
