...
class Code
  class Object
    class JobFailedExecution < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::JobFailedExecution).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::JobFailedExecution).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::JobFailedExecution.column_names).empty?
