...
class Code
  class Object
    class JobPause < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::JobPause).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::JobPause).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::JobPause.column_names).empty?
          raise ::Code::Error, "invalid_record_attributes"
        end

        Pundit
          .policy_scope!(::Current.user, ::JobPause)
          .where(attributes)
          .to_code
      end
      ...
      def self.code_find(value)
        ...
        Pundit.policy_scope!(::Current.user, ::JobPause).find_by(id: id).to_code
      end

      def self.code_find!(value)
        id = value.to_code.to_s
        Pundit.policy_scope!(::Current.user, ::JobPause).find(id).to_code
      end
    end
  end
end
