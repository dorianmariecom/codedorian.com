...
class Code
  class Object
    class JobContext < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::JobContext).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::JobContext).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::JobContext.column_names).empty?
      ...
      def self.code_find!(value)
        id = value.to_code.to_s
        Pundit.policy_scope!(::Current.user, ::JobContext).find(id).to_code
      end
    end
  end
end
