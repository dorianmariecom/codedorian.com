...
class Code
  class Object
    class Password < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::Password).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::Password).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::Password.column_names).empty?
          raise ::Code::Error, "invalid_record_attributes"
        end

        Pundit
          .policy_scope!(::Current.user, ::Password)
          .where(attributes)
          .to_code
      end
      ...
      def self.code_find(value)
        ...
        Pundit.policy_scope!(::Current.user, ::Password).find_by(id: id).to_code
      end

      def self.code_find!(value)
        id = value.to_code.to_s
        Pundit.policy_scope!(::Current.user, ::Password).find(id).to_code
      end
      ...
      def password! = policy_scope(::Password).find(id)
      def code_user = policy_scope(::User).find(password!.user_id).to_code
