...
class Code
  class Object
    class TimeZone < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::TimeZone).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::TimeZone).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::TimeZone.column_names).empty?
          raise ::Code::Error, "invalid_record_attributes"
        end

        Pundit
          .policy_scope!(::Current.user, ::TimeZone)
          .where(attributes)
          .to_code
      end
      ...
      def self.code_find(value)
        ...
        Pundit.policy_scope!(::Current.user, ::TimeZone).find_by(id: id).to_code
      end

      def self.code_find!(value)
        id = value.to_code.to_s
        Pundit.policy_scope!(::Current.user, ::TimeZone).find(id).to_code
      end
