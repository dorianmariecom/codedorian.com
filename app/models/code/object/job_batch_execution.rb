...
class Code
  class Object
    class JobBatchExecution < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::JobBatchExecution).find(
          code_get("id").to_s
        )
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::JobBatchExecution).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::JobBatchExecution.column_names).empty?
