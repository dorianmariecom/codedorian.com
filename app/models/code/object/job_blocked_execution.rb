...
class Code
  class Object
    class JobBlockedExecution < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::JobBlockedExecution).find(
          code_get("id").to_s
        )
      end
      ...
      def code_semaphore
        Pundit
          ...
          .find_by(id: record!.semaphore&.id)
          .to_code
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::JobBlockedExecution).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::JobBlockedExecution.column_names).empty?
