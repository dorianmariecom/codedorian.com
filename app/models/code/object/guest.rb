...
class Code
  class Object
    class Guest < Dictionary
      def record!
        Pundit.policy_scope!(::Current.user, ::Guest).find(code_get("id").to_s)
      end
      ...
      def self.code_all
        Pundit.policy_scope!(::Current.user, ::Guest).to_code
      end

      def self.code_where(value)
        attributes = value.to_code.as_json
        unless (attributes.keys - ::Guest.column_names).empty?
          raise ::Code::Error, "invalid_record_attributes"
        end

        Pundit.policy_scope!(::Current.user, ::Guest).where(attributes).to_code
      end

      def self.code_find(value)
        id = value.to_code.to_s
        Pundit.policy_scope!(::Current.user, ::Guest).find_by(id: id).to_code
      end

      def self.code_find!(value)
        id = value.to_code.to_s
        Pundit.policy_scope!(::Current.user, ::Guest).find(id).to_code
      end
    end
  end
end
