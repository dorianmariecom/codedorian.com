...
class Step < ApplicationRecord
  ...
  def self.search_fields
    { input: { node: -> { arel_table[:input] }, type: :string }, position: { node: -> { arel_table[:position] }, type: :integer }, offset_seconds: { node: -> { arel_table[:offset_seconds] }, type: :integer }, **base_search_fields }
  end
