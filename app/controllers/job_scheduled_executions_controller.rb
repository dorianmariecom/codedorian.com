...
class JobScheduledExecutionsController < ApplicationController
  ...
  before_action do
    add_breadcrumb(key: "job_scheduled_executions.index", path: index_url)
  end
  before_action(
    :load_job_scheduled_execution,
  ...
  def id
    params[:job_scheduled_execution_id].presence || params[:id]
  end
  ...
  def load_job_scheduled_execution
    @job_scheduled_execution = authorize(scope.find(id))
    set_context(job_scheduled_execution: @job_scheduled_execution)
    add_breadcrumb(text: @job_scheduled_execution, path: show_url)
  end

  def job_scheduled_execution_params
    if admin?
      params.expect(
        job_scheduled_execution: %i[job_id queue_name priority scheduled_at]
      )
    else
      {}
    end
