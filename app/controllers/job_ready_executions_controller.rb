...
class JobReadyExecutionsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.json do
        render(json: { status: :ok, messages: [], data: @job_ready_executions })
      end
    end
  end
  ...
  def show
    ...
    respond_to do |format|
      format.html
      format.json do
        render(json: { status: :ok, messages: [], data: @job_ready_execution })
      end
    end
  end
        ...
        format.json do
          render(
            json: {
              ...
              messages: [@job_ready_execution.alert],
              data: @job_ready_execution
            },
            status: :unprocessable_content
          )
