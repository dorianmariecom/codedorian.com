...
class ErrorsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.json { render(json: { status: :ok, messages: [], data: @errors }) }
    end
  end
  ...
  def show
    ...
    respond_to do |format|
      format.html
      format.json { render(json: { status: :ok, messages: [], data: @error }) }
    end
  end
  ...
  def create
    ...
    if @error.save(context: :controller)
      respond_to do |format|
        ...
        format.json do
          render(json: { status: :ok, messages: [t(".notice")], data: @error })
        end
      end
    else
