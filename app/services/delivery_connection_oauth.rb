...
class DeliveryConnectionOauth
  PROVIDERS = %w[slack x reddit mastodon facebook gmail google_workspace outlook].freeze
  ERRORS = [SlackOauth::Error, XOauth::Error, RedditOauth::Error, MastodonOauth::Error, FacebookOauth::Error, MailboxOauth::Error].freeze

  def self.pending(user:, provider:, redirect_uri:, server: nil)
    pending = {
  ...
  def self.authorization_url(pending:, redirect_uri:)
    ...
    when "gmail", "google_workspace", "outlook"
      MailboxOauth.new(pending.fetch("provider")).authorization_url(state: state, verifier: verifier, redirect_uri: redirect_uri)
    end
  end
  ...
  def self.exchange(pending:, code:, redirect_uri:)
    ...
    when "gmail", "google_workspace", "outlook"
      [MailboxOauth.new(pending.fetch("provider")).exchange(code: code, verifier: pending.fetch("verifier"), redirect_uri: redirect_uri)]
    end
  end
end
