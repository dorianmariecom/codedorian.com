...
class XOauth
  SCOPES = %w[tweet.read tweet.write users.read dm.read dm.write offline.access].freeze
  ...
  def self.exchange(code:, verifier:, redirect_uri:)
    ...
    attributes.merge(sender: user["id"], name: "X · @#{user['username']}", enabled: true)
  rescue KeyError
    raise Error
  end
  ...
  def self.token_attributes(data)
    unless data["token_type"].to_s.casecmp?("bearer") && data["access_token"].present? &&
             data["refresh_token"].present? && data["expires_in"].is_a?(Integer) && data["expires_in"].positive? &&
