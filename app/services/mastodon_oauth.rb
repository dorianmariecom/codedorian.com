...
class MastodonOauth
  ...
  def self.origin(server)
    ...
    uri =
      URI.parse(
        server.include?("://") ? server.strip : "https://#{server.strip}"
      )
  ...
  def self.register(server:, redirect_uri:)
    ...
    data =
      post(
        base_url,
        "/api/v1/apps",
        client_name: "codedorian.com",
        redirect_uris: redirect_uri,
        scopes: SCOPES.join(" ")
      )
    unless data["client_id"].is_a?(String) && data["client_id"].present? &&
             data["client_secret"].is_a?(String) &&
             data["client_secret"].present?
      raise Error
    end

    {
      ...
      "client_secret" =>
        ActiveRecord::Encryption.encryptor.encrypt(data["client_secret"])
    }
  end
      ...
      post(
        ...
        client_secret:
          ActiveRecord::Encryption.encryptor.decrypt(
            registration.fetch("client_secret")
          ),
        redirect_uri: redirect_uri,
        scope: SCOPES.join(" ")
      )
    unless data["access_token"].is_a?(String) &&
             data["access_token"].present? &&
             data["token_type"].to_s.casecmp?("bearer") &&
  ...
  def self.request_json(uri, request)
    ...
    data
  rescue DeliveryAdapters::Rejected,
         Resolv::ResolvError,
         JSON::ParserError,
         IOError,
         SystemCallError,
