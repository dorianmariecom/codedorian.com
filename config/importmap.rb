# frozen_string_literal: true

pin "@codemirror/autocomplete", to: "@codemirror--autocomplete.js" # @6.20.0
pin "@codemirror/commands", to: "@codemirror--commands.js" # @6.10.1
pin "@codemirror/language", to: "@codemirror--language.js" # @6.12.1
pin "@codemirror/lint", to: "@codemirror--lint.js" # @6.9.2
pin "@codemirror/search", to: "@codemirror--search.js" # @6.6.0
pin "@codemirror/state", to: "@codemirror--state.js" # @6.5.4
pin "@codemirror/view", to: "@codemirror--view.js" # @6.39.11
pin "@googlemaps/js-api-loader", to: "@googlemaps--js-api-loader.js" # @2.0.2
pin "@hotwired/hotwire-native-bridge", to: "@hotwired--hotwire-native-bridge.js" # @1.2.2
pin "@hotwired/stimulus", to: "@hotwired--stimulus.js" # @3.2.2
pin "@hotwired/stimulus-loading", to: "stimulus-loading.js", integrity: true
pin "@hotwired/turbo", to: "@hotwired--turbo.js" # @8.0.21
pin "@hotwired/turbo-rails", to: "@hotwired--turbo-rails.js" # @8.0.21
pin "@lezer/common", to: "@lezer--common.js" # @1.5.0
pin "@lezer/highlight", to: "@lezer--highlight.js" # @1.2.3
pin "@marijn/find-cluster-break", to: "@marijn--find-cluster-break.js" # @1.0.2
pin "@rails/actioncable", to: "@rails--actioncable.js" # @8.1.200
pin "@rails/actioncable/src", to: "@rails--actioncable--src.js" # @8.1.200
pin "@rails/actiontext", to: "@rails--actiontext.js" # @8.1.200
pin "@sentry-internal/browser-utils", to: "@sentry-internal--browser-utils.js" # @10.34.0
pin "@sentry-internal/feedback", to: "@sentry-internal--feedback.js" # @10.34.0
pin "@sentry-internal/replay", to: "@sentry-internal--replay.js" # @10.34.0
pin "@sentry-internal/replay-canvas", to: "@sentry-internal--replay-canvas.js" # @10.34.0
pin "@sentry/browser", to: "@sentry--browser.js" # @10.34.0
pin "@sentry/core", to: "@sentry--core.js" # @10.34.0
pin "application", integrity: true
pin "codemirror" # @6.0.2
pin "constants", integrity: true
pin "consumer" # @1.2.2
pin "crelt" # @1.0.6
pin "debounce", integrity: true
pin "i18n", integrity: true
pin "intl-tel-input" # @25.15.0
pin "intl-tel-input/build/js/utils.js",
    to: "intl-tel-input--build--js--utils.js.js" # @25.14.1
pin "local-time" # @3.0.3
pin "style-mod" # @4.1.3
pin "thememirror" # @2.0.1
pin "trix" # @2.1.16
pin "w3c-keyname" # @2.2.8
pin_all_from "app/javascript/controllers", under: "controllers", integrity: true
