...
class Text {
  ...
  replace(e, t, n) {
    ...
    return TextNode.from(i, this.length - (t - e) + n.length);
  }
  append(e) {
  ...
  eq(e) {
    ...
    let i = new RawTextCursor(this),
      s = new RawTextCursor(e);
    for (let e = t, r = t; ; ) {
      i.next(e);
      s.next(e);
      e = 0;
      if (i.lineBreak != s.lineBreak || i.done != s.done || i.value != s.value)
        return false;
      r += i.value.length;
...
class TextLeaf extends Text {
  ...
  lineInner(e, t, n, i) {
    for (let s = 0; ; s++) {
      let r = this.text[s],
        a = i + r.length;
      if ((t ? n : a) >= e) return new Line(i, a, n, r);
      i = a + 1;
      n++;
    }
  }
  decompose(e, t, n, i) {
    let s =
      e <= 0 && t >= this.length
        ? this
        : new TextLeaf(
            sliceText(this.text, e, t),
            Math.min(t, this.length) - Math.max(0, e),
          );
    if (i & 1) {
      let e = n.pop();
      let t = appendText(s.text, e.text.slice(), 0, s.length);
      if (t.length <= 32) n.push(new TextLeaf(t, e.length + s.length));
      else {
        let e = t.length >> 1;
  ...
  replace(e, t, n) {
    ...
    let i = appendText(
      this.text,
      appendText(n.text, sliceText(this.text, 0, e)),
      t,
    );
    let s = this.length + n.length - (t - e);
    return i.length <= 32
      ? new TextLeaf(i, s)
      : TextNode.from(TextLeaf.split(i, []), s);
  }
  sliceString(e, t = this.length, n = "\n") {
    ...
    for (let s = 0, r = 0; s <= t && r < this.text.length; r++) {
      let a = this.text[r],
        o = s + a.length;
      s > e && r && (i += n);
      e < o && t > s && (i += a.slice(Math.max(0, e - s), t - s));
      s = o + 1;
    }
    return i;
  }
  ...
  static split(e, t) {
    ...
    for (let s of e) {
      ...
      i += s.length + 1;
      if (n.length == 32) {
        t.push(new TextLeaf(n, i));
        n = [];
        i = -1;
      }
    }
    i > -1 && t.push(new TextLeaf(n, i));
    return t;
  }
}
class TextNode extends Text {
  ...
  lineInner(e, t, n, i) {
    for (let s = 0; ; s++) {
      let r = this.children[s],
        a = i + r.length,
        o = n + r.lines - 1;
      if ((t ? o : a) >= e) return r.lineInner(e, t, n, i);
      i = a + 1;
      n = o + 1;
    }
  }
  decompose(e, t, n, i) {
    for (let s = 0, r = 0; r <= t && s < this.children.length; s++) {
      let a = this.children[s],
        o = r + a.length;
      if (e <= o && t >= r) {
        let s = i & ((r <= e ? 1 : 0) | (o >= t ? 2 : 0));
        r >= e && o <= t && !s ? n.push(a) : a.decompose(e - r, t - r, n, s);
      }
      r = o + 1;
    }
  }
    ...
    if (n.lines < this.lines)
      for (let i = 0, s = 0; i < this.children.length; i++) {
        let r = this.children[i],
          a = s + r.length;
        if (e >= s && t <= a) {
          let o = r.replace(e - s, t - s, n);
          let l = this.lines - r.lines + o.lines;
          if (o.lines < l >> 4 && o.lines > l >> 6) {
            let s = this.children.slice();
            s[i] = o;
            return new TextNode(s, this.length - (t - e) + n.length);
          }
  ...
  sliceString(e, t = this.length, n = "\n") {
    ...
    for (let s = 0, r = 0; s < this.children.length && r <= t; s++) {
      let a = this.children[s],
        o = r + a.length;
      r > e && s && (i += n);
      e < o && t > r && (i += a.sliceString(e - r, t - r, n));
      r = o + 1;
    }
    return i;
  }
  ...
  static from(e, t = e.reduce((e, t) => e + t.length + 1, -1)) {
    ...
    function add(e) {
      ...
      else if (e.lines > r && (o > r || !o)) {
        flush();
        a.push(e);
      } else if (
        e instanceof TextLeaf &&
        o &&
        (t = h[h.length - 1]) instanceof TextLeaf &&
        e.lines + t.lines <= 32
      ) {
        o += e.lines;
        l += e.length + 1;
    ...
    function flush() {
      if (o != 0) {
        a.push(h.length == 1 ? h[0] : TextNode.from(h, l));
        l = -1;
        o = h.length = 0;
      }
    }
    ...
    flush();
    return a.length == 1 ? a[0] : new TextNode(a, t);
  }
}
...
function appendText(e, t, n = 0, i = 1e9) {
  for (let s = 0, r = 0, a = true; r < e.length && s <= i; r++) {
    let o = e[r],
      l = s + o.length;
    if (l >= n) {
      l > i && (o = o.slice(0, i - s));
  ...
  nextInner(e, t) {
    ...
    for (;;) {
      ...
      let i = this.nodes[n],
        s = this.offsets[n],
        r = s >> 1;
      let a = i instanceof TextLeaf ? i.text.length : i.children.length;
      if (r == (t > 0 ? a : 0)) {
        if (n == 0) {
          this.done = true;
          this.value = "";
        ...
        let s = i.text[r + (t < 0 ? -1 : 0)];
        this.offsets[n] += t;
        if (s.length > Math.max(0, e)) {
          this.value =
            e == 0 ? s : t > 0 ? s.slice(e) : s.slice(0, s.length - e);
          return this;
        }
        e -= s.length;
      } else {
        let s = i.children[r + (t < 0 ? -1 : 0)];
        if (e > s.length) {
          e -= s.length;
          this.offsets[n] += t;
        } else {
          ...
          this.offsets.push(
            t > 0
              ? 1
              : (s instanceof TextLeaf ? s.text.length : s.children.length) <<
                  1,
          );
        }
      }
...
function findClusterBreak(t, n, i = true, s = true) {
  return e(t, n, i, s);
}
...
function codePointAt(e, t) {
  ...
  return surrogateLow(i) ? i - 56320 + ((n - 55296) << 10) + 65536 : n;
}
  ...
  touchesRange(e, t = e) {
    for (let n = 0, i = 0; n < this.sections.length && i <= t; ) {
      let s = this.sections[n++],
        r = this.sections[n++],
        a = i + s;
      if (r >= 0 && i <= t && a >= e) return !(i < e && a > t) || "cover";
      i = a;
    }
    return false;
  }
...
class ChangeSet extends ChangeDesc {
  ...
  apply(e) {
    ...
    iterChanges(
      this,
      (t, n, i, s, r) => (e = e.replace(i, i + (n - t), r)),
      false,
    );
    return e;
  }
  mapDesc(e, t = false) {
    return mapSet(this, e, t, true);
  }
  invert(e) {
    ...
    for (let i = 0, s = 0; i < t.length; i += 2) {
      ...
      if (a >= 0) {
        ...
        t[i + 1] = r;
        let o = i >> 1;
        while (n.length < o) n.push(Text.empty);
        n.push(r ? e.slice(s, s + r) : Text.empty);
      }
      s += r;
    }
  ...
  compose(e) {
    return this.empty ? e : e.empty ? this : composeSets(this, e, true);
  }
  map(e, t = false) {
    return e.empty ? this : mapSet(this, e, t, true);
  }
  ...
  filter(e) {
    ...
    e: for (let r = 0, a = 0; ; ) {
      ...
      while (a < o || (a == o && s.len == 0)) {
        if (s.done) break e;
        let e = Math.min(s.len, o - a);
        addSection(i, e, -1);
        let r = s.ins == -1 ? -1 : s.off == 0 ? s.ins : 0;
        addSection(t, e, r);
        r > 0 && addInsert(n, t, s.text);
        s.forward(e);
        a += e;
      }
      let l = e[r++];
      while (a < l) {
        if (s.done) break e;
        let e = Math.min(s.len, l - a);
        addSection(t, e, -1);
        addSection(i, e, s.ins == -1 ? -1 : s.off == 0 ? s.ins : 0);
        s.forward(e);
        a += e;
      }
    }
    return { changes: new ChangeSet(t, n), filtered: ChangeDesc.create(i) };
  }
  toJSON() {
    let e = [];
    for (let t = 0; t < this.sections.length; t += 2) {
  ...
  static of(e, n, i) {
    let s = [],
      ...
      a = 0;
    let o = null;
    function flush(e = false) {
      if (!e && !s.length) return;
      a < n && addSection(s, n - a, -1);
      let t = new ChangeSet(s, r);
      o = o ? o.compose(t.map(o)) : t;
      s = [];
      r = [];
      a = 0;
    }
    function process(e) {
      if (Array.isArray(e)) for (let t of e) process(t);
      else if (e instanceof ChangeSet) {
        if (e.length != n)
          throw new RangeError(
            `Mismatched change set length (got ${e.length}, expected ${n})`,
          );
        flush();
        o = o ? o.compose(e.map(o)) : e;
      } else {
        let { from: o, to: l = o, insert: h } = e;
        if (o > l || o < 0 || l > n)
          throw new RangeError(
            `Invalid change range ${o} to ${l} (in doc of length ${n})`,
          );
        let c = h
          ? typeof h == "string"
            ? Text.of(h.split(i || t))
    ...
    for (let i = 0; i < e.length; i++) {
      ...
      if (typeof s == "number") t.push(s, -1);
        ...
        if (
          !Array.isArray(s) ||
          typeof s[0] != "number" ||
          s.some((e, t) => t && typeof e != "string")
        )
          throw new RangeError("Invalid JSON representation of ChangeSet");
        if (s.length == 1) t.push(s[0], 0);
        else {
          while (n.length < i) n.push(Text.empty);
          n[i] = Text.of(s.slice(1));
          t.push(s[0], n[i].length);
        }
      }
    }
    return new ChangeSet(t, n);
...
function addSection(e, t, n, i = false) {
  if (t == 0 && n <= 0) return;
  let s = e.length - 2;
  if (s >= 0 && n <= 0 && n == e[s + 1]) e[s] += t;
  else if (s >= 0 && t == 0 && e[s] == 0) e[s + 1] += n;
  else if (i) {
    e[s] += t;
    e[s + 1] += n;
  } else e.push(t, n);
...
function addInsert(e, t, n) {
  ...
  if (i < e.length) e[e.length - 1] = e[e.length - 1].append(n);
    ...
    while (e.length < i) e.push(Text.empty);
    e.push(n);
  }
}
function iterChanges(e, t, n) {
  let i = e.inserted;
  for (let s = 0, r = 0, a = 0; a < e.sections.length; ) {
    let o = e.sections[a++],
      l = e.sections[a++];
    if (l < 0) {
      s += o;
      ...
      for (;;) {
        h += o;
        c += l;
        l && i && (f = f.append(i[(a - 2) >> 1]));
        if (n || a == e.sections.length || e.sections[a + 1] < 0) break;
        o = e.sections[a++];
        l = e.sections[a++];
      }
      t(s, h, r, c, f);
...
function mapSet(e, t, n, i = false) {
  ...
  for (let e = -1; ; ) {
    ...
    if (a.ins == -1 && o.ins == -1) {
      ...
      (a.ins < 0 ||
        e == a.i ||
        (a.off == 0 && (o.len < a.len || (o.len == a.len && !n))))
    ) {
      let t = o.len;
      addSection(s, o.ins, -1);
      while (t) {
        let n = Math.min(a.len, t);
        if (a.ins >= 0 && e < a.i && a.len <= n) {
          addSection(s, 0, a.ins);
          r && addInsert(r, s, a.text);
          e = a.i;
        }
        a.forward(n);
        t -= n;
      ...
      }
      {
        ...
        while (n)
          if (o.ins == -1) {
            ...
            if (!(o.ins == 0 && o.len < n)) break;
            n -= o.len;
            o.next();
          }
        addSection(s, t, e < a.i ? a.ins : 0);
        r && e < a.i && addInsert(r, s, a.text);
        e = a.i;
        a.forward(a.len - n);
      }
    }
...
function composeSets(e, t, n = false) {
  let i = [];
  let s = n ? [] : null;
  let r = new SectionIter(e),
    a = new SectionIter(t);
  for (let e = false; ; ) {
    if (r.done && a.done)
      return s ? ChangeSet.createSet(i, s) : ChangeDesc.create(i);
    if (r.ins == 0) {
      addSection(i, r.len, 0, e);
      r.next();
    } else if (a.len != 0 || a.done) {
      if (r.done || a.done) throw new Error("Mismatched change set lengths");
      {
        let t = Math.min(r.len2, a.len),
          n = i.length;
        if (r.ins == -1) {
          let n = a.ins == -1 ? -1 : a.off ? 0 : a.ins;
          addSection(i, t, n, e);
          s && n && addInsert(s, i, a.text);
        } else if (a.ins == -1) {
          addSection(i, r.off ? 0 : r.len, t, e);
          s && addInsert(s, i, r.textBit(t));
        } else {
          addSection(i, r.off ? 0 : r.len, a.off ? 0 : a.ins, e);
          s && !a.off && addInsert(s, i, a.text);
        }
        e = (r.ins > t || (a.ins >= 0 && a.len > t)) && (e || i.length > n);
        r.forward2(t);
        a.forward(t);
      }
    } else {
      addSection(i, 0, a.ins, e);
      s && addInsert(s, i, a.text);
      a.next();
    }
  }
}
class SectionIter {
  ...
  forward2(e) {
    if (this.ins == -1) this.forward(e);
    else if (e == this.ins) this.next();
    else {
      this.ins -= e;
      this.off += e;
    }
...
class SelectionRange {
  ...
  map(e, t = -1) {
    let n, i;
    if (this.empty) n = i = e.mapPos(this.from, t);
    else {
      n = e.mapPos(this.from, 1);
      i = e.mapPos(this.to, -1);
    }
    return n == this.from && i == this.to
      ? this
      : new SelectionRange(n, i, this.flags);
  }
  extend(e, t = e) {
    if (e <= this.anchor && t >= this.anchor)
      return EditorSelection.range(e, t);
...
class EditorSelection {
  ...
  eq(e, t = false) {
    ...
    for (let n = 0; n < this.ranges.length; n++)
      if (!this.ranges[n].eq(e.ranges[n], t)) return false;
    return true;
  }
  get main() {
  ...
  replaceRange(e, t = this.mainIndex) {
    ...
    n[t] = e;
    return EditorSelection.create(n, this.mainIndex);
  }
  toJSON() {
  ...
  static create(e, t = 0) {
    ...
    for (let n = 0, i = 0; i < e.length; i++) {
      ...
      if (s.empty ? s.from <= n : s.from < n)
        return EditorSelection.normalized(e.slice(), t);
      n = s.to;
    }
  ...
  static range(e, t, n, i) {
    ...
    return t < e
      ? SelectionRange.create(t, e, 48 | s)
      : SelectionRange.create(e, t, (t > e ? 8 : 0) | s);
  }
  static normalized(e, t = 0) {
    ...
    for (let n = 1; n < e.length; n++) {
      ...
      if (i.empty ? i.from <= s.to : i.from < s.to) {
        let r = s.from,
          a = Math.max(i.to, s.to);
        n <= t && t--;
        e.splice(
          --n,
          2,
          i.anchor > i.head
            ? EditorSelection.range(a, r)
            : EditorSelection.range(r, a),
        );
      }
    }
    return new EditorSelection(e, t);
  ...
  dynamicSlot(e) {
    ...
    for (let n of this.dependencies)
      n == "doc"
        ...
        : n == "selection"
          ? (l = true)
          : (((t = e[n.id]) !== null && t !== void 0 ? t : 1) & 1) == 0 &&
            h.push(e[n.id]);
    return {
      create(e) {
        e.values[r] = n(e);
      ...
      update(e, t) {
        if (
          ...
          let t = n(e);
          if (a ? !compareArray(t, e.values[r], i) : !i(t, e.values[r])) {
            e.values[r] = t;
            return 1;
          }
        }
            ...
            this.dependencies.every((n) =>
              n instanceof Facet
                ...
                : !(n instanceof StateField) ||
                  t.field(n, false) == e.field(n, false),
            ) ||
            (a ? compareArray((o = n(e)), s, i) : i((o = n(e)), s))
          ) {
            e.values[r] = s;
            return 0;
          }
...
function compareArray(e, t, n) {
  ...
  for (let i = 0; i < e.length; i++) if (!n(e[i], t[i])) return false;
  return true;
}
function ensureAll(e, t) {
  let n = false;
  for (let i of t) ensureAddr(e, i) & 1 && (n = true);
  return n;
}
function dynamicFacetSlot(e, t, n) {
  let i = n.map((t) => e[t.id]);
  let s = n.map((e) => e.type);
  let r = i.filter((e) => !(e & 1));
  let a = e[t.id] >> 1;
  function get(e) {
    let n = [];
    for (let t = 0; t < i.length; t++) {
      let r = getAddr(e, i[t]);
      if (s[t] == 2) for (let e of r) n.push(e);
      else n.push(r);
    }
    return t.combine(n);
  }
  return {
    ...
    reconfigure(e, s) {
      let r = ensureAll(e, i);
      let o = s.config.facets[t.id],
        l = s.facet(t);
      if (o && !r && sameArray(n, o)) {
        e.values[a] = l;
        return 0;
      }
      let h = get(e);
...
const s = Facet.define({ static: true });
class StateField {
  ...
  static define(e) {
    ...
    e.provide && (t.provides = e.provide(t));
    return t;
  }
  create(e) {
    let t = e.facet(s).find((e) => e.field == this);
    return ((t === null || t === void 0 ? void 0 : t.create) || this.createF)(
      e,
    );
  }
  slot(e) {
    ...
    return {
      ...
      reconfigure: (e, n) => {
        let i,
          ...
          a = n.facet(s);
        if (
          (i = r.find((e) => e.field == this)) &&
          i != a.find((e) => e.field == this)
        ) {
          e.values[t] = i.create(e);
          return 1;
        }
        if (n.config.address[this.id] != null) {
          e.values[t] = n.field(this);
          return 0;
        }
        e.values[t] = this.create(e);
        return 1;
  ...
  init(e) {
    return [this, s.of({ field: this, create: e })];
  }
  get extension() {
    return this;
  }
}
const r = { lowest: 4, low: 3, default: 2, high: 1, highest: 0 };
function prec(e) {
  return (t) => new PrecExtension(t, e);
}
const a = {
  highest: prec(r.highest),
  high: prec(r.high),
  default: prec(r.default),
  low: prec(r.low),
  lowest: prec(r.lowest),
};
class PrecExtension {
  constructor(e, t) {
    this.inner = e;
...
class Configuration {
  ...
  static resolve(e, t, n) {
    ...
    let r = new Map();
    for (let n of flatten(e, t, r))
      n instanceof StateField
        ? i.push(n)
        : (s[n.facet.id] || (s[n.facet.id] = [])).push(n);
    ...
    let h = n === null || n === void 0 ? void 0 : n.config.facets;
    for (let e in s) {
      let t = s[e],
        i = t[0].facet;
      let r = (h && h[e]) || [];
      if (t.every((e) => e.type == 0)) {
        a[i.id] = (o.length << 1) | 1;
        if (sameArray(r, t)) o.push(n.facet(i));
        else {
          let e = i.combine(t.map((e) => e.value));
          o.push(n && i.compare(e, n.facet(i)) ? n.facet(i) : e);
        }
      } else {
        for (let e of t)
          if (e.type == 0) {
        ...
        a[i.id] = l.length << 1;
        l.push((e) => dynamicFacetSlot(e, i, t));
      }
    }
    let c = l.map((e) => e(a));
    return new Configuration(e, r, c, a, o, s);
  }
}
function flatten(e, t, n) {
  ...
  function inner(e, a) {
    ...
    s.set(e, a);
    if (Array.isArray(e)) for (let t of e) inner(t, a);
      ...
      i[a].push(e);
      e.provides && inner(e.provides, a);
    } else if (e instanceof FacetProvider) {
      i[a].push(e);
      e.facet.extensions && inner(e.facet.extensions, r.default);
    } else {
      let t = e.extension;
      if (!t)
        throw new Error(
  ...
  inner(e, r.default);
  return i.reduce((e, t) => e.concat(t));
}
...
const o = Facet.define();
const l = Facet.define({ combine: (e) => e.some((e) => e), static: true });
const h = Facet.define({
  combine: (e) => (e.length ? e[0] : void 0),
  static: true,
});
const c = Facet.define();
const f = Facet.define();
const u = Facet.define();
const d = Facet.define({ combine: (e) => !!e.length && e[0] });
class Annotation {
  constructor(e, t) {
    this.type = e;
    this.value = t;
...
class Transaction {
  constructor(e, t, n, i, s, r) {
    ...
    this._doc = null;
    this._state = null;
    n && checkSelection(n, t.newLength);
    s.some((e) => e.type == Transaction.time) ||
      (this.annotations = s.concat(Transaction.time.of(Date.now())));
  }
  static create(e, t, n, i, s, r) {
    return new Transaction(e, t, n, i, s, r);
  }
  get newDoc() {
...
function joinRanges(e, t) {
  ...
  for (let i = 0, s = 0; ; ) {
    let r, a;
    if (i < e.length && (s == t.length || t[s] >= e[i])) {
      r = e[i++];
      a = e[i++];
    } else {
      if (!(s < t.length)) return n;
      r = t[s++];
      a = t[s++];
    }
    !n.length || n[n.length - 1] < r
      ? n.push(r, a)
      : n[n.length - 1] < a && (n[n.length - 1] = a);
  }
}
function mergeTransaction(e, t, n) {
  ...
  if (n) {
    s = t.changes;
    r = ChangeSet.empty(t.changes.length);
    a = e.changes.compose(t.changes);
  } else {
    s = t.changes.map(e.changes);
    r = e.changes.mapDesc(t.changes, true);
    a = e.changes.compose(s);
  }
  return {
    changes: a,
    selection: t.selection
      ? t.selection.map(r)
      : (i = e.selection) === null || i === void 0
        ? void 0
        : i.map(s),
    effects: StateEffect.mapEffects(e.effects, s).concat(
      StateEffect.mapEffects(t.effects, r),
    ),
...
function resolveTransactionInner(e, t, n) {
  let i = t.selection,
    s = asArray(t.annotations);
  t.userEvent && (s = s.concat(Transaction.userEvent.of(t.userEvent)));
  return {
    changes:
      t.changes instanceof ChangeSet
        ? t.changes
        : ChangeSet.of(t.changes || [], n, e.facet(h)),
    selection:
      i &&
...
function resolveTransaction(e, t, n) {
  let i = resolveTransactionInner(e, t.length ? t[0] : {}, e.doc.length);
  t.length && t[0].filter === false && (n = false);
  for (let s = 1; s < t.length; s++) {
    t[s].filter === false && (n = false);
    let r = !!t[s].sequential;
    i = mergeTransaction(
      i,
      resolveTransactionInner(e, t[s], r ? i.changes.newLength : e.doc.length),
      r,
    );
  }
  let s = Transaction.create(
    e,
    i.changes,
    i.selection,
    i.effects,
  ...
  return extendTransaction(n ? filterTransaction(s) : s);
}
function filterTransaction(e) {
  let t = e.startState;
  let n = true;
  for (let i of t.facet(c)) {
    let t = i(e);
    if (t === false) {
      n = false;
      break;
    }
    Array.isArray(t) && (n = n === true ? t : joinRanges(n, t));
  }
  if (n !== true) {
    let i, s;
    if (n === false) {
      s = e.changes.invertedDesc;
      i = ChangeSet.empty(t.doc.length);
    } else {
      let t = e.changes.filter(n);
      i = t.changes;
      s = t.filtered.mapDesc(t.changes).invertedDesc;
    }
  ...
  for (let n = i.length - 1; n >= 0; n--) {
    ...
    e =
      s instanceof Transaction
        ? s
        : Array.isArray(s) && s.length == 1 && s[0] instanceof Transaction
          ? s[0]
          : resolveTransaction(t, asArray(s), false);
  }
  return e;
}
function extendTransaction(e) {
  let t = e.startState,
    n = t.facet(u),
    i = e;
  for (let s = n.length - 1; s >= 0; s--) {
    ...
    r &&
      ...
      (i = mergeTransaction(
        i,
        resolveTransactionInner(t, r, e.changes.newLength),
        true,
      ));
  }
  return i == e
...
const g = [];
function asArray(e) {
  return e == null ? g : Array.isArray(e) ? e : [e];
}
var p = (function (e) {
  e[(e.Word = 0)] = "Word";
  e[(e.Space = 1)] = "Space";
...
let S;
try {
  S = new RegExp("[\\p{Alphabetic}\\p{Number}_]", "u");
} catch (e) {}
function hasWordChar(e) {
  if (S) return S.test(e);
  for (let t = 0; t < e.length; t++) {
    let n = e[t];
    if (
      /\w/.test(n) ||
      (n > "" && (n.toUpperCase() != n.toLowerCase() || m.test(n)))
    )
      return true;
  }
  return false;
...
function makeCategorizer(e) {
  return (t) => {
    if (!/\S/.test(t)) return p.Space;
    if (hasWordChar(t)) return p.Word;
    for (let n = 0; n < e.length; n++) if (t.indexOf(e[n]) > -1) return p.Word;
    return p.Other;
  };
}
class EditorState {
  ...
  field(e, t = true) {
    ...
    if (n != null) {
      ensureAddr(this, n);
      return getAddr(this, n);
    }
    if (t) throw new RangeError("Field is not present in this state");
  }
  update(...e) {
    return resolveTransaction(this, e, true);
  }
  applyTransaction(e) {
    let t = this.config,
      { base: n, compartments: i } = t;
    for (let s of e.effects)
      if (s.is(Compartment.reconfigure)) {
        if (t) {
          i = new Map();
          t.compartments.forEach((e, t) => i.set(t, e));
          t = null;
        }
        i.set(s.value.compartment, s.value.extension);
      } else if (s.is(StateEffect.reconfigure)) {
        t = null;
        n = s.value;
      } else if (s.is(StateEffect.appendConfig)) {
        t = null;
        n = asArray(n).concat(s.value);
      }
    let s;
    if (t) s = e.startState.values.slice();
    else {
      t = Configuration.resolve(n, i, this);
      let e = new EditorState(
        t,
        this.doc,
        this.selection,
    ...
    let r = e.startState.facet(l) ? e.newSelection : e.newSelection.asSingle();
    new EditorState(t, e.newDoc, r, s, (t, n) => n.update(t, e), e);
  }
  replaceSelection(e) {
  ...
  changeByRange(e) {
    ...
    let i = this.changes(n.changes),
      s = [n.range];
    let r = asArray(n.effects);
    for (let n = 1; n < t.ranges.length; n++) {
      let a = e(t.ranges[n]);
      let o = this.changes(a.changes),
        l = o.map(i);
      for (let e = 0; e < n; e++) s[e] = s[e].map(l);
      let h = i.mapDesc(o, true);
      s.push(a.range.map(h));
      i = i.compose(l);
      r = StateEffect.mapEffects(r, l).concat(
        StateEffect.mapEffects(asArray(a.effects), h),
      );
    }
    return {
      changes: i,
      selection: EditorSelection.create(s, t.mainIndex),
      effects: r,
    };
  }
  changes(e = []) {
  ...
  toText(e) {
    return Text.of(e.split(this.facet(EditorState.lineSeparator) || t));
  }
  ...
  facet(e) {
    ...
    if (t == null) return e.default;
    ensureAddr(this, t);
    return getAddr(this, t);
  }
  toJSON(e) {
    ...
    if (n)
      for (let t in n)
        if (Object.prototype.hasOwnProperty.call(e, t)) {
          let s = n[t],
            r = e[t];
          i.push(s.init((e) => s.spec.fromJSON(r, e)));
        }
  ...
  static create(e = {}) {
    ...
    let i =
      e.doc instanceof Text
        ? e.doc
        : Text.of(
            (e.doc || "").split(n.staticFacet(EditorState.lineSeparator) || t),
          );
    let s = e.selection
    ...
    checkSelection(s, i.length);
    n.staticFacet(l) || (s = s.asSingle());
  ...
  wordAt(e) {
    ...
    let r = e - n,
      a = e - n;
    while (r > 0) {
      let e = findClusterBreak(t, r, false);
      if (s(t.slice(e, r)) != p.Word) break;
      r = e;
    }
    while (a < i) {
      let e = findClusterBreak(t, a);
      if (s(t.slice(a, e)) != p.Word) break;
      a = e;
    }
    return r == a ? null : EditorSelection.range(r + n, a + n);
  }
}
EditorState.allowMultipleSelections = l;
EditorState.tabSize = Facet.define({ combine: (e) => (e.length ? e[0] : 4) });
EditorState.lineSeparator = h;
EditorState.readOnly = d;
...
EditorState.languageData = o;
EditorState.changeFilter = c;
EditorState.transactionFilter = f;
EditorState.transactionExtender = u;
...
function combineConfig(e, t, n = {}) {
  ...
  for (let e in t) i[e] === void 0 && (i[e] = t[e]);
  return i;
}
class RangeValue {
  eq(e) {
...
RangeValue.prototype.mapMode = n.TrackDel;
class Range {
  constructor(e, t, n) {
    this.from = e;
    this.to = t;
...
class Chunk {
  ...
  findIndex(e, t, n, i = 0) {
    ...
    for (let r = i, a = s.length; ; ) {
      if (r == a) return r;
      let i = (r + a) >> 1;
      let o =
        s[i] - e || (n ? this.value[i].endSide : this.value[i].startSide) - t;
      if (i == r) return o >= 0 ? r : a;
      o >= 0 ? (a = i) : (r = i + 1);
    }
  }
  between(e, t, n, i) {
    for (
      let s = this.findIndex(t, -1e9, true),
        r = this.findIndex(n, 1e9, false, s);
      s < r;
      s++
    )
      if (i(this.from[s] + e, this.to[s] + e, this.value[s]) === false)
        return false;
  }
  map(e, t) {
    let n = [],
    ...
    for (let o = 0; o < this.value.length; o++) {
      let l,
        ...
        u = this.to[o] + e;
      if (f == u) {
        let e = t.mapPos(f, c.startSide, c.mapMode);
        if (e == null) continue;
        l = h = e;
        if (c.startSide != c.endSide) {
          h = t.mapPos(f, c.endSide);
          if (h < l) continue;
        }
      } else {
        l = t.mapPos(f, c.startSide);
        h = t.mapPos(u, c.endSide);
        if (l > h || (l == h && c.startSide > 0 && c.endSide <= 0)) continue;
      }
      if (!((h - l || c.endSide - c.startSide) < 0)) {
        r < 0 && (r = l);
        c.point && (a = Math.max(a, h - l));
        n.push(c);
        i.push(l - r);
        s.push(h - r);
      }
    }
    return { mapped: n.length ? new Chunk(i, s, n, a) : null, pos: r };
  }
}
class RangeSet {
  constructor(e, t, n, i) {
  ...
  update(e) {
    ...
    n && (t = t.slice().sort(cmpRange));
    if (this.isEmpty) return t.length ? RangeSet.of(t) : this;
    let a = new LayerCursor(this, null, -1).goto(0),
      o = 0,
      l = [];
    let h = new RangeSetBuilder();
    while (a.value || o < t.length)
      if (
        o < t.length &&
        ...
        (!r || i > a.to || s < a.from || r(a.from, a.to, a.value)) &&
          (h.addInner(a.from, a.to, a.value) ||
            l.push(Range.create(a.from, a.to, a.value)));
        a.next();
      }
  ...
  map(e) {
    ...
    for (let s = 0; s < this.chunk.length; s++) {
      let r = this.chunkPos[s],
        a = this.chunk[s];
      let o = e.touchesRange(r, r + a.length);
      if (o === false) {
        i = Math.max(i, a.maxPoint);
        t.push(a);
        n.push(e.mapPos(r));
      } else if (o === true) {
        let { mapped: s, pos: o } = a.map(r, e);
        if (s) {
    ...
    return t.length == 0 ? s : new RangeSet(n, t, s || RangeSet.empty, i);
  }
  between(e, t, n) {
    if (!this.isEmpty) {
  ...
  static compare(e, t, n, i, s = -1) {
    let r = e.filter((e) => e.maxPoint > 0 || (!e.isEmpty && e.maxPoint >= s));
    let a = t.filter((e) => e.maxPoint > 0 || (!e.isEmpty && e.maxPoint >= s));
    let o = findSharedChunks(r, a, n);
    let l = new SpanCursor(r, o, s);
    let h = new SpanCursor(a, o, s);
    n.iterGaps((e, t, n) => compare(l, e, h, t, n, i));
    n.empty && n.length == 0 && compare(l, 0, h, 0, 0, i);
  }
  static eq(e, t, n = 0, i) {
    i == null && (i = 999999999);
    let s = e.filter((e) => !e.isEmpty && t.indexOf(e) < 0);
    let r = t.filter((t) => !t.isEmpty && e.indexOf(t) < 0);
    if (s.length != r.length) return false;
    if (!s.length) return true;
    let a = findSharedChunks(s, r);
    let o = new SpanCursor(s, a, 0).goto(n),
      l = new SpanCursor(r, a, 0).goto(n);
    for (;;) {
      if (
        o.to != l.to ||
        !sameValues(o.active, l.active) ||
        (o.point && (!l.point || !o.point.eq(l.point)))
      )
        return false;
      if (o.to > i) return true;
      o.next();
      l.next();
    }
  }
  static spans(e, t, n, i, s = -1) {
    let r = new SpanCursor(e, null, s).goto(t),
      a = t;
    let o = r.openStart;
    for (;;) {
        ...
        let s =
          r.pointFrom < t
            ...
            : r.point.startSide < 0
              ? n.length
              : Math.min(n.length, o);
        i.point(a, e, r.point, n, s, r.pointRank);
        o = Math.min(r.openEnd(e), n.length);
      } else if (e > a) {
        i.span(a, e, r.active, o);
        o = r.openEnd(e);
      }
  ...
  static of(e, t = false) {
    ...
    for (let i of e instanceof Range ? [e] : t ? lazySort(e) : e)
  ...
  static join(e) {
    ...
    for (let n = e.length - 2; n >= 0; n--)
      for (let i = e[n]; i != RangeSet.empty; i = i.nextLayer)
        t = new RangeSet(
          i.chunkPos,
          i.chunk,
          t,
          Math.max(i.maxPoint, t.maxPoint),
        );
    return t;
  }
}
...
function lazySort(e) {
  if (e.length > 1)
    for (let t = e[0], n = 1; n < e.length; n++) {
      let i = e[n];
      if (cmpRange(t, i) > 0) return e.slice().sort(cmpRange);
      t = i;
    }
  return e;
}
...
class RangeSetBuilder {
  ...
  addChunk(e, t) {
    if ((e - this.lastTo || t.value[0].startSide - this.last.endSide) < 0)
      return false;
    this.from.length && this.finishChunk(true);
  ...
  finishInner(e) {
    this.from.length && this.finishChunk(false);
    if (this.chunks.length == 0) return e;
    let t = RangeSet.create(
      this.chunkPos,
      this.chunks,
...
function findSharedChunks(e, t, n) {
  ...
  for (let e of t)
    for (let t = 0; t < e.chunk.length; t++) {
      let r = i.get(e.chunk[t]);
      r == null ||
        (n ? n.mapPos(r) : r) != e.chunkPos[t] ||
        (n === null || n === void 0
          ? void 0
          : n.touchesRange(r, r + e.chunk[t].length)) ||
        s.add(e.chunk[t]);
    }
  return s;
}
  ...
  static from(e, t = null, n = -1) {
    ...
    for (let s = 0; s < e.length; s++)
      for (let r = e[s]; !r.isEmpty; r = r.nextLayer)
        r.maxPoint >= n && i.push(new LayerCursor(r, t, n, s));
    return i.length == 1 ? i[0] : new HeapCursor(i);
  }
  get startSide() {
    return this.value ? this.value.startSide : 0;
  }
...
function heapBubble(e, t) {
  for (let n = e[t]; ; ) {
    ...
    if (i >= e.length) break;
    let s = e[i];
    if (i + 1 < e.length && s.compare(e[i + 1]) >= 0) {
      s = e[i + 1];
      i++;
    }
...
class SpanCursor {
  ...
  addActive(e) {
    ...
    while (
      ...
      (s - this.activeRank[t] || i - this.activeTo[t]) > 0
    )
      t++;
    insert(this.active, t, n);
    insert(this.activeTo, t, i);
    insert(this.activeRank, t, s);
...
function compare(e, t, n, i, s, r) {
  ...
  for (;;) {
    ...
    let s = i < 0 ? e.to + l : n.to,
      h = Math.min(s, a);
    e.point || n.point
      ? (e.point &&
          n.point &&
          (e.point == n.point || e.point.eq(n.point)) &&
          sameValues(e.activeForPoint(e.to), n.activeForPoint(n.to))) ||
        r.comparePoint(o, h, e.point, n.point)
      : h > o &&
        !sameValues(e.active, n.active) &&
        r.compareRange(o, h, e.active, n.active);
    if (s > a) break;
    (t || e.openEnd != n.openEnd) && r.boundChange && r.boundChange(s);
    o = s;
    i <= 0 && e.next();
    i >= 0 && n.next();
  }
...
function sameValues(e, t) {
  ...
  for (let n = 0; n < e.length; n++)
    if (e[n] != t[n] && !e[n].eq(t[n])) return false;
  return true;
}
function remove(e, t) {
...
function findMinIndex(e, t) {
  ...
  for (let s = 0; s < t.length; s++)
    if ((t[s] - i || e[s].endSide - e[n].endSide) < 0) {
      n = s;
      i = t[s];
    }
  return n;
...
function countColumn(e, t, n = e.length) {
  let i = 0;
  for (let s = 0; s < n && s < e.length; )
    if (e.charCodeAt(s) == 9) {
      i += t - (i % t);
      s++;
    } else {
      i++;
      s = findClusterBreak(e, s);
    }
  return i;
}
function findColumn(e, t, n, i) {
  for (let i = 0, s = 0; ; ) {
    if (s >= t) return i;
    if (i == e.length) break;
    s += e.charCodeAt(i) == 9 ? n - (s % n) : 1;
    i = findClusterBreak(e, i);
  }
  return i === true ? -1 : e.length;
}
export {
  Annotation,
  AnnotationType,
