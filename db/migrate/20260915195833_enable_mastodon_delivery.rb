...
class EnableMastodonDelivery < ActiveRecord::Migration[8.1]
  def up
    execute <<~SQL.squish
      ...
      VALUES ('mastodon', TRUE, 0, 'eur', TRUE, TRUE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
  ...
  def down
    execute <<~SQL.squish
      UPDATE delivery_channels SET enabled = FALSE, amount_cents = NULL,
        updated_at = CURRENT_TIMESTAMP WHERE key = 'mastodon'
    SQL
  end
end
