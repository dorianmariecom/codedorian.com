...
class ApplicationHelperTest < ActionView::TestCase
  test "describes every supported schedule interval in both locales" do
    ScheduleConcern::INTERVALS.product(%i[en fr]).each do |interval, locale|
      I18n.with_locale(locale) do
        schedule = PlanSchedule.new(
          interval: interval,
          starts_at: Time.utc(2026, 8, 4, 10)
        )

        assert_predicate(plan_schedule_description(schedule), :present?, interval)
      end
    end
  end
  ...
  test "describes calendar intervals using the viewer time zone" do
    Time.use_zone("America/Los_Angeles") do
      assert_equal(
        "every tuesday at 3:00am",
        description_for("1 week")
      )
      assert_equal(
