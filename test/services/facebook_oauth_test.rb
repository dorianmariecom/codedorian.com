...
class FacebookOauthTest < ActiveSupport::TestCase
  setup do
    ...
    stub_request(:get, %r{/debug_token}).to_return(
      body: {
        data: {
          is_valid: true,
          type: "USER",
    ...
    stub_request(:get, %r{/me/permissions}).to_return(
      body: {
        data:
          FacebookOauth::SCOPES.map do |permission|
  ...
  test "explicit publishing grants recover pages omitted by the accounts endpoint" do
    ...
    stub_request(:get, "https://graph.facebook.com/v25.0/debug_token").with(
      query: {
      ...
      body: {
        data: {
          ...
          app_id: "client",
          granular_scopes: [
            { scope: "pages_manage_posts", target_ids: ["1"] },
            { scope: "pages_messaging", target_ids: ["2"] }
          ]
        }
      }.to_json
    )
    stub_request(:get, "https://graph.facebook.com/v25.0/1").with(
      query: hash_including("fields" => "id,name,access_token"),
      headers: {
  ...
  test "invalid or cross app tokens cannot discover selected pages" do
    stub_request(:get, %r{/me/accounts}).to_return(body: { data: [] }.to_json)
    [
      { is_valid: false, type: "USER", app_id: "client" },
      { is_valid: true, type: "USER", app_id: "other" }
    ].each do |metadata|
      stub_request(:get, %r{/debug_token}).to_return(
        body: { data: metadata }.to_json
      )
  ...
  test "pagination stays on graph host and skips pages without publishing access" do
    first = {
      data: [page, page.merge(id: "2", tasks: ["ANALYZE"])],
      paging: {
    ...
    stub_request(:get, %r{/me/accounts}).with(
      query: hash_including("after" => "cursor")
  ...
  test "new page experience publishing tasks are accepted but analytics alone is not" do
    %w[
      ...
      stub_request(:get, %r{/me/accounts}).to_return(
        body: { data: [page.merge(tasks: [task])] }.to_json
      )
      assert_equal ["1"], exchange.pluck(:sender)
    end
    stub_request(:get, %r{/me/accounts}).to_return(
      body: { data: [page.merge(tasks: ["PROFILE_PLUS_ANALYZE"])] }.to_json
    )
    assert_equal "no_pages",
                 assert_raises(FacebookOauth::Error) { exchange }.reason
  end
  ...
  test "missing permissions malformed and empty page results are rejected" do
    [
      ...
      "not json",
      { data: [] }.to_json,
      { data: [page.merge(access_token: nil)] }.to_json,
      { data: [page.merge(tasks: ["ANALYZE"])] }.to_json,
      { data: [page], paging: "invalid" }.to_json
    ].each do |body|
    ...
    stub_request(:get, %r{/me/permissions}).to_return(
      body: {
        data: [{ permission: "pages_manage_posts", status: "declined" }]
      }.to_json
    )
    assert_raises(FacebookOauth::Error) { exchange }
  end
  ...
  test "missing permissions and unavailable pages have actionable reasons" do
    ...
    assert_equal "missing_permissions",
                 assert_raises(FacebookOauth::Error) { exchange }.reason
  end
  ...
  def exchange
    FacebookOauth.exchange(
      code: "code",
      redirect_uri: "https://codedorian.com/facebook_connections/callback"
    )
  end
end
