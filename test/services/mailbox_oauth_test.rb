...
class MailboxOauthTest < ActiveSupport::TestCase
  ...
  test "refresh preserves an omitted refresh token and accepts rotation" do
    ...
    %w[gmail google_workspace outlook].each do |provider|
      connection = DeliveryConnection.create!(provider: provider, name: "Mailbox", smtp_from: "sender@example.com", access_token: "old", refresh_token: "original", token_expires_at: 1.minute.ago)
      endpoint = provider == "outlook" ? "https://login.microsoftonline.com/common/oauth2/v2.0/token" : "https://oauth2.googleapis.com/token"
      refresh_tokens.each do |refresh|
        ...
        stub_request(:post, endpoint).with do |request|
          ...
          true
        end.to_return(body: { access_token: "new", refresh_token: refresh, token_type: "bearer", expires_in: 3600 }.compact.to_json)
        assert_equal "new", MailboxOauth.new(provider).access_token_for(connection)
  ...
  test "revoked tokens require reconnection without clearing existing credentials" do
    connection = DeliveryConnection.create!(provider: "gmail", name: "Mailbox", smtp_from: "sender@example.com", access_token: "old", refresh_token: "refresh", token_expires_at: 1.minute.ago)
    stub_request(:post, "https://oauth2.googleapis.com/token").to_return(status: 400, body: { error: "invalid_grant" }.to_json)
    error = assert_raises(MailboxOauth::Error) { MailboxOauth.new("gmail").access_token_for(connection) }
  ...
  test "temporary token failures are retryable and disabled connections never refresh" do
    connection = DeliveryConnection.create!(provider: "gmail", name: "Mailbox", smtp_from: "sender@example.com", access_token: "old", refresh_token: "refresh", token_expires_at: 1.minute.ago)
    stub_request(:post, "https://oauth2.googleapis.com/token").to_return(status: 503)
    error = assert_raises(MailboxOauth::Error) { MailboxOauth.new("gmail").access_token_for(connection) }
  ...
  test "tokens missing send permission are not persisted" do
    connection = DeliveryConnection.create!(provider: "outlook", name: "Mailbox", smtp_from: "sender@example.com", access_token: "old", refresh_token: "refresh", token_expires_at: 1.minute.ago)
    stub_request(:post, "https://login.microsoftonline.com/common/oauth2/v2.0/token").to_return(body: { access_token: "new", expires_in: 3600, token_type: "Bearer", scope: "User.Read" }.to_json)
    error = assert_raises(MailboxOauth::Error) { MailboxOauth.new("outlook").access_token_for(connection) }
