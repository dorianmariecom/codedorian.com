...
class SlackRecipientTest < ActiveSupport::TestCase
  ...
  test "channel names resolve through paginated channel listings and are cached" do
    first = stub_request(:get, "https://slack.com/api/conversations.list")
      .with(query: { limit: 200, types: "public_channel,private_channel", exclude_archived: true })
      .to_return(body: { ok: true, channels: [], response_metadata: { next_cursor: "next" } }.to_json)
    stub_request(:get, "https://slack.com/api/conversations.list")
      .with(query: { limit: 200, types: "public_channel,private_channel", exclude_archived: true, cursor: "next" })
      .to_return(body: { ok: true, channels: [{ id: "C123", name: "dorian" }] }.to_json)
    2.times { assert_equal "C123", SlackRecipient.resolve(@connection, "#dorian") }
    assert_requested first, times: 1
  end

  test "user handles open a direct conversation before posting" do
    stub_request(:get, "https://slack.com/api/users.list").with(query: { limit: 200 })
      .to_return(body: { ok: true, members: [{ id: "U123", name: "dorian" }] }.to_json)
    opened = stub_request(:post, "https://slack.com/api/conversations.open")
      .with(body: { users: "U123" }, headers: { "Authorization" => "Bearer test-token" })
  ...
  test "ambiguous display names are rejected without opening a conversation" do
    stub_request(:get, "https://slack.com/api/users.list").with(query: { limit: 200 })
      .to_return(body: { ok: true, members: [
        { id: "U1", name: "one", profile: { display_name: "dorian" } },
        { id: "U2", name: "two", profile: { display_name: "dorian" } }
      ] }.to_json)
    error = assert_raises(DeliveryAdapters::Rejected) { SlackRecipient.resolve(@connection, "@dorian") }
  ...
  test "bare strings and missing names are rejected before network access" do
    ["dorian", "C123/invalid", "@", "#", "@two people"].each do |recipient|
      error = assert_raises(DeliveryAdapters::Rejected) { SlackRecipient.resolve(@connection, recipient) }
