...
class NewDeliveryAdaptersTest < ActiveSupport::TestCase
    ...
    %w[gmail google_workspace].each_with_index do |provider, index|
      ...
      sent =
        stub_request(
          ...
          .with do |request|
            ...
            mail =
              Mail.read_from_string(
                Base64.urlsafe_decode64(JSON.parse(request.body).fetch("raw"))
              )
          ...
          .to_return(body: { id: "gmail-id" }.to_json)
      ...
      stub_request(:post, "https://graph.microsoft.com/v1.0/me/sendMail")
        .with do |request|
          ...
          assert_equal "Bonjour",
                       Mail.read_from_string(
                         Base64.strict_decode64(request.body)
                       ).subject
          true
        end
    ...
    sent =
      stub_request(:post, "https://api.sendgrid.com/v3/mail/send")
        .with do |request|
          ...
          assert_equal [{ "to" => [{ "email" => "recipient@example.com" }] }],
                       body["personalizations"]
          assert_equal [
            { "type" => "text/plain", "value" => "Le monde" },
            { "type" => "text/html", "value" => "<p>Le monde</p>" }
          ],
                       body["content"]
          true
        end
  ...
  test "Resend preserves content and uses stable idempotency" do
    configure("resend")
    sent =
      stub_request(:post, "https://api.resend.com/emails").with(
        headers: {
  ...
  test "Mailgun uses the selected regional endpoint and form fields" do
    ...
    %w[us eu].each do |region|
      ...
      sent =
        stub_request(
          :post,
          "https://#{host}/v3/mail.example.com/messages"
        ).with(
  ...
  test "Mailchimp sends transactional mail and rejects unsuccessful recipient results" do
    ...
    sent =
      stub_request(:post, "https://mandrillapp.com/api/1.0/messages/send.json")
        ...
        .to_return(body: [{ status: "queued", _id: "mandrill-id" }].to_json)
    assert_equal "mandrill-id", DeliveryAdapters.deliver(@delivery).provider_id
    assert_requested sent
    stub_request(
      :post,
      "https://mandrillapp.com/api/1.0/messages/send.json"
    ).to_return(body: [{ status: "rejected", _id: "id" }].to_json)
    error =
    ...
    sent =
      stub_request(
        ...
        .with do |request|
          ...
          body = JSON.parse(request.body)
          assert_equal ["recipient@example.com"],
                       body["Destination"]["ToAddresses"]
          assert_equal "Bonjour",
                       Mail.read_from_string(
                         Base64.strict_decode64(body["Content"]["Raw"]["Data"])
                       ).subject
          true
        end
        .to_return(body: { MessageId: "ses-id" }.to_json)
  ...
  test "SES signs temporary credentials and rejects missing message identifiers" do
    ...
    sent =
      stub_request(
        ...
        .to_return(body: { MessageId: "" }.to_json)
    assert_raises(IOError) { DeliveryAdapters.deliver(@delivery) }
    assert_requested sent
  end
    ...
    %w[messenger instagram].each do |provider|
      ...
      sent =
        stub_request(:post, "https://#{host}/v26.0/sender-id/messages")
          ...
          .to_return(body: { message_id: "meta-id" }.to_json)
  ...
  test "Telegram sends numeric chat ids and handles application throttling" do
    ...
    sent =
      stub_request(
        ...
        .to_return(body: { ok: true, result: { message_id: 123 } }.to_json)
    assert_equal "123", DeliveryAdapters.deliver(@delivery).provider_id
    assert_requested sent
    stub_request(
      :post,
      "https://api.telegram.org/bot123:bot-token/sendMessage"
    ).to_return(body: { ok: false, error_code: 429 }.to_json)
    error =
      assert_raises(DeliveryAdapters::Rejected) do
        DeliveryAdapters.deliver(@delivery)
      end
  ...
  test "Viber validates status even on HTTP success" do
    ...
    sent =
      stub_request(:post, "https://chatapi.viber.com/pa/send_message")
        .with do |request|
        ...
        .to_return(body: { status: 0, message_token: 987 }.to_json)
    assert_equal "987", DeliveryAdapters.deliver(@delivery).provider_id
    assert_requested sent
    stub_request(:post, "https://chatapi.viber.com/pa/send_message").to_return(
      body: { status: 6, status_message: "not subscribed" }.to_json
    )
    error =
      assert_raises(DeliveryAdapters::Rejected) do
        DeliveryAdapters.deliver(@delivery)
  ...
  test "all new providers reject auth and rate limits and preserve unknown outcomes" do
    ...
    endpoints.each do |provider, endpoint|
      configure(
        ...
        channel:
          (
            if provider.in?(%w[messenger instagram telegram viber])
              provider
            else
              "email"
            end
      ...
      statuses.each do |status|
        ...
        error =
          assert_raises(DeliveryAdapters::Rejected, provider) do
            DeliveryAdapters.deliver(@delivery)
          end
        assert_equal status == 429, error.retryable
      end
      stub_request(:post, endpoint).to_timeout
      assert_raises(Timeout::Error, provider) do
        DeliveryAdapters.deliver(@delivery)
      end
      next if provider.in?(%w[outlook sendgrid])

      stub_request(:post, endpoint).to_return(body: "broken json")
      assert_raises(JSON::ParserError, provider) do
        DeliveryAdapters.deliver(@delivery)
      end
    end
  end
  ...
  test "Meta reports provider rejection codes and Viber rate limits retry" do
    ...
    stub_request(
      ...
      body: {
        error: {
          code: 10,
          message: "Outside conversation window"
        }
      }.to_json
    )
    error =
  ...
  test "unverified email never reaches any provider" do
    ...
    assert_not_requested :post,
                         /gmail|microsoft|amazonaws|sendgrid|resend|mailgun|mandrill/
  end
  ...
  def configure(provider, channel: "email")
    ...
    @delivery.assign_attributes(
      ...
      channel: channel,
      recipient: channel == "email" ? "recipient@example.com" : "12345"
    )
  end
end
