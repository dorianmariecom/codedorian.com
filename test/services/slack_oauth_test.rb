...
class SlackOauthTest < ActiveSupport::TestCase
  ...
  test "malformed missing permission and expiring tokens are rejected" do
    ...
    invalid_responses << oauth_response.merge(
      authed_user: {
        id: "U123"
      }
    ).to_json
    invalid_responses << oauth_response.merge(
      authed_user: oauth_response[:authed_user].merge(expires_in: 43_200)
    ).to_json
  ...
  def exchange
    SlackOauth.exchange(
      code: "code",
      redirect_uri: "https://codedorian.com/slack_connections/callback"
    )
  end
end
