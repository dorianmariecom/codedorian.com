...
class DeliveryChannelConfigurationTest < ActiveSupport::TestCase
  ...
  test "database visibility and patterns control destination validation" do
    channel = DeliveryChannel.create!(key: "messages", only: "public", show_recipient: true,
                                      public_pattern: "#[a-z]+", private_pattern: "@[a-z]+")
    destination = DeliveryDestination.new(delivery_channel: channel, recipient: "#hello", visibility: "private")
  ...
  test "optional recipients and invalid channel patterns are handled explicitly" do
    channel = DeliveryChannel.new(key: "messages", show_recipient: true, public_pattern: "(@[a-z]+)?", private_pattern: "@[a-z]+")
  ...
  test "messenger uses a regular recipient and shared delivery connection" do
    ...
    channel = DeliveryChannel.create!(key: "messenger", only: "private", private_pattern: "[0-9]+",
                                      delivery_connection: connection, enabled: true, amount_cents: 0, show_recipient: true)
    destination = DeliveryDestination.create!(user: subscriptions(:subscription).user, delivery_channel: channel, recipient: "789")
    delivery = Delivery.create!(subscription: subscriptions(:subscription), delivery_destination: destination, event_key: "messenger")
  ...
  test "deleting a personal connection leaves its destinations unavailable" do
    connection = DeliveryConnection.create!(provider: "slack", name: "Slack", access_token: "token")
    channel = DeliveryChannel.create!(key: "slack", enabled: true, amount_cents: 0)
    destination = DeliveryDestination.create!(delivery_channel: channel, delivery_connection: connection, recipient: "#hello")
    assert destination.available?
    connection.destroy!
