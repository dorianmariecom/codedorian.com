...
class DeliveryProvidersTest < ActiveSupport::TestCase
  ...
  test "email supports all email providers but rejects messaging connections" do
    channel = DeliveryChannel.new(key: "email", enabled: true, amount_cents: 120)
    %w[smtp gmail google_workspace outlook aws_ses sendgrid resend mailgun mailchimp].each do |provider|
      connection = DeliveryConnection.create!(provider: provider, name: provider)
      channel.delivery_connection = connection
      assert channel.valid?, channel.errors.full_messages.join(", ")
      assert_not channel.available?
    end
    channel.delivery_connection = DeliveryConnection.create!(provider: "telegram", name: "Bot", access_token: "123:token")
    assert_not channel.valid?
  end

  test "new provider accounts must belong to admins and secrets remain encrypted" do
    connection = DeliveryConnection.new(provider: "aws_ses", name: "SES", user: users(:other_user), aws_secret_access_key: "secret-value", aws_session_token: "session-value")
    assert_not connection.valid?
    connection.user = users(:admin)
    connection.save!
    assert_not_includes connection.aws_secret_access_key_before_type_cast, "secret-value"
    assert_not_includes connection.aws_session_token_before_type_cast, "session-value"
  ...
  test "changing email provider preserves destinations prices and queued connection snapshots" do
    smtp = DeliveryConnection.create!(provider: "smtp", name: "SMTP", smtp_from: "sender@example.com", smtp_address: "smtp.example.com")
  ...
  test "provider-specific required fields and invalid regions prevent readiness" do
    ses = DeliveryConnection.new(user: users(:admin), provider: "aws_ses", name: "SES", smtp_from: "sender@example.com", aws_access_key_id: "key", aws_region: "eu-west-1")
    assert_not ses.ready?
    ses.aws_secret_access_key = "secret"
    assert ses.ready?
    ses.aws_region = "https://other.example"
    assert_not ses.valid?
    mailgun = DeliveryConnection.new(user: users(:admin), provider: "mailgun", name: "Mailgun", smtp_from: "sender@example.com", api_key: "key", mailgun_domain: "mail.example.com")
