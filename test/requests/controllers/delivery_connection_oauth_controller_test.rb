...
class DeliveryConnectionOauthControllerTest < ActionDispatch::IntegrationTest
  ...
  test "logging out during impersonation clears pending oauth before restoring the admin" do
    ...
    delete login_path
    get callback_delivery_connections_path(provider: "slack"),
        params: {
          state: state,
          code: "code"
        }
    assert_redirected_to delivery_connections_path
    assert_equal I18n.t("delivery_connections.callback.invalid_state"),
                 flash[:alert]
  ...
  test "oauth state cannot be transferred between providers" do
    ...
    state =
      URI.decode_www_form(URI(response.location).query).to_h.fetch("state")
    get callback_delivery_connections_path(provider: "x"),
        params: {
          state: state,
          code: "code"
        }
