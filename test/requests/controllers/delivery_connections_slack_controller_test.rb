...
class DeliveryConnectionsSlackControllerTest < ActionDispatch::IntegrationTest
  ...
  test "connect reconnect list and disconnect with full owned data" do
    2.times do |index|
      ...
      assert_difference "DeliveryConnection.count", index.zero? ? 2 : 0 do
        get callback_delivery_connections_path(provider: "slack", locale: nil), params: { state: state, code: "code" }
  ...
  test "invalid state and replay do not exchange tokens" do
    state = start_connection
    get callback_delivery_connections_path(provider: "slack", locale: nil), params: { state: "invalid", code: "code" }
    get callback_delivery_connections_path(provider: "slack", locale: nil), params: { state: state, code: "code" }
  ...
  test "denial and provider failure leave no connection" do
    state = start_connection
    get callback_delivery_connections_path(provider: "slack", locale: nil), params: { state: state, error: "access_denied" }
    assert_not_requested :post, "https://slack.com/api/oauth.v2.access"
    state = start_connection
  ...
  def start_connection
    ...
    assert_equal "#{Current.base_url}/delivery_connections/callback/slack", query["redirect_uri"]
    query.fetch("state")
  end
  ...
  def stub_exchange(body = { ok: true, token_type: "bot", scope: "chat:write", access_token: "bot-token", bot_user_id: "B123", team: { id: "T123", name: "Workspace" }, authed_user: { id: "U123", scope: "chat:write", access_token: "user-token" } })
    stub_request(:post, "https://slack.com/api/oauth.v2.access")
      .with(basic_auth: %w[slack-client slack-secret], body: { code: "code", redirect_uri: "#{Current.base_url}/delivery_connections/callback/slack" })
