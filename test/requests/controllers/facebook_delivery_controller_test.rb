...
class FacebookDeliveryControllerTest < ActionDispatch::IntegrationTest
  setup do
    @channel = Current.with(user: users(:admin)) do
      ...
      DeliveryChannel.create!(key: "facebook", only: "public", public_pattern: "(?:)", show_recipient: false, enabled: true, amount_cents: 0, delivery_connection: connection, show_visibility: true)
    end
  ...
  test "destination and subscription forms show Facebook without exposing the page token" do
    [new_delivery_destination_path,
     new_service_subscription_path(services(:service), plan_id: plans(:plan).id)].each do |path|
      ...
      assert_select "option[data-delivery-destination-form-only=public]", minimum: 1
      assert_select "[data-delivery-destination-form-target=facebookHelp]", count: 0
