...
class DeliveryConnectionsMailboxControllerTest < ActionDispatch::IntegrationTest
    ...
    %w[gmail google_workspace outlook].each do |provider|
      ...
      2.times do |index|
        ...
        stub_request(:post, endpoint).with do |request|
          ...
          assert_equal "#{Current.base_url}/delivery_connections/callback/#{provider}", data["redirect_uri"]
        ...
        assert_difference "DeliveryConnection.count", index.zero? ? 1 : 0 do
          get callback_delivery_connections_path(provider: provider, locale: nil), params: { state: query["state"], code: "auth-code" }
  ...
  test "invalid expired replayed denied and unsupported authorizations cannot connect" do
    ...
    get callback_delivery_connections_path(provider: "gmail", locale: nil), params: { state: "wrong", code: "code" }
    get callback_delivery_connections_path(provider: "gmail", locale: nil), params: { state: query["state"], code: "code" }
    query = start_connection("gmail")
    travel 11.minutes do
      get callback_delivery_connections_path(provider: "gmail", locale: nil), params: { state: query["state"], code: "code" }
    end
    query = start_connection("gmail")
    get callback_delivery_connections_path(provider: "gmail", locale: nil), params: { state: query["state"], error: "access_denied" }
  ...
  test "regular users and guests cannot manage mailbox connections" do
    ...
    get delivery_connections_path, as: :json
    assert_response :success
    post connect_delivery_connections_path(provider: "gmail"), params: { provider: "gmail" }, as: :json
    assert_response :bad_request
    delete login_path
    post connect_delivery_connections_path(provider: "gmail"), params: { provider: "outlook" }, as: :json
    assert_response :bad_request
  end
  ...
  def start_connection(provider)
    post connect_delivery_connections_path(provider: provider), params: { provider: provider }
