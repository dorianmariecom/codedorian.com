...
class MastodonConnectionsControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    stub_request(:post, "https://mastodon.example/api/v1/apps")
      .to_return(body: { client_id: "client", client_secret: "client-secret" }.to_json)
    stub_request(:post, "https://mastodon.example/oauth/token")
      .with(body: hash_including("client_id" => "client", "client_secret" => "client-secret", "grant_type" => "authorization_code"))
      .to_return(body: { access_token: "access-secret", token_type: "Bearer", scope: MastodonOauth::SCOPES.join(" ") }.to_json)
    stub_request(:get, "https://mastodon.example/api/v1/accounts/verify_credentials")
  ...
  test "connect reconnect list and disconnect keep tokens private" do
    ...
    connection = DeliveryConnection.where_user(users(:other_user)).where(provider: "mastodon").sole
